...
const getStatName = (arr, minBaseStat) => {
  const filteredStats = arr.filter((stat) => stat.baseStat > minBaseStat);
  const statNames = filteredStats.map((stat) => stat.stat.name);
  return statNames;
...
const getCharactersWithoutChildren = (arr) => {
  const filteredCharacters = arr.filter((character) => !character.children);
...
const evenOddNumericValues = (arr) => {
  ...
  const mappedArr = filteredArr.map((value) =>
    value % 2 === 0 ? "even" : "odd"
  );
  return mappedArr;
