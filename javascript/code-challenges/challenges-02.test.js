...
const raisedToTheThird = (arr) => {
  ...
  arr.forEach((number) => {
    const raised = Math.pow(number, 3);
    result.push(raised);
  });
...
const addOne = (arr) => {
  const incrementedArray = arr.map((number) => number + 1);
...
const forEachTwoToThe = (arr) => {
  ...
  arr.forEach((number) => {
    const raised = Math.pow(2, number);
    result.push(raised);
  });
...
const mapTwoToThe = (arr) => {
  const result = arr.map((number) => Math.pow(2, number));
  return result;
};
...
const charCode = (arr) => {
  const charCodeArray = arr.map((letter) => letter.charCodeAt(0));
...
const extractAbilities = (arr) => {
  const abilityNames = arr.map((ability) => ability.ability.name);
  return abilityNames;
};
